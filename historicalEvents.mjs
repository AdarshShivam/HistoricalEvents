...
export const getHistoricalEvents = (inputDateTime, maxLength) => {
  // Validate maxLength
  if (typeof maxLength !== "number" || maxLength <= 0) {
    throw new Error("Invalid maxLength value");
  }
  // Basic input validation
  else if (!inputDateTime || isNaN(new Date(inputDateTime))) {
    ...
    if (matchingEvents.length === 0) {
      throw new Error("No historical events found for the specified date and time");
    }
    // Return historical events
    return matchingEvents.filter((_, i) => i < maxLength);
  }
};
